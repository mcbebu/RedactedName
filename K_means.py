...
def k_means_constrained(file_path, min_orders, max_orders, num_drivers):
    ...
    print(updated_data)
    propogator.fit_predict(updated_data)

    return updated_data, propogator.cluster_centers_ ,propogator.labels_
...
def create_proximity_map(data, labels) :
    t3 = np.hstack((data,labels.reshape((labels.size, 1))))
